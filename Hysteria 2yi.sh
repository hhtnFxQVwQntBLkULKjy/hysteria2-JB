...
EOF
    ...
    print_success "客户端配置文件已生成: /root/hysteria2-client.yaml"
    
    # 显示连接信息 - 简洁列表格式
    echo ""
    echo -e "${CYAN}🔗 客户端连接信息:${NC}"
    echo -e "${GREEN}服务器地址:${NC} $server_address"
    echo -e "${GREEN}连接密码:${NC} $password"
    echo -e "${GREEN}客户端带宽:${NC} 上行 1 Gbps / 下行 1 Gbps"
    echo -e "${GREEN}SOCKS5 代理:${NC} 127.0.0.1:1080"
    echo -e "${GREEN}HTTP 代理:${NC} 127.0.0.1:8080"
    
    # 显示端口跳跃信息
    if [[ -n "$port_hopping_info" ]]; then
        echo -e "${YELLOW}$port_hopping_info${NC}"
    fi
